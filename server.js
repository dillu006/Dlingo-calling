...
//   1. Push this folder to a GitHub repo: server.js, package.json, index.html
//      (all three directly in the repo root — no subfolders needed)
...
const PUBLIC_DIR = __dirname; // index.html lives at the repo root
const WS_MAGIC = '258EAFA5-E914-47DA-95CA-C5AB0DC85B11';
// Don't let visitors download the server's own source files
const BLOCKED_FILES = new Set(['/server.js', '/package.json', '/README.md', '/.git']);

// ---- Tiny static file server (serves index.html and friends from the repo root) ----
const MIME = { '.html': 'text/html', '.js': 'text/javascript', '.css': 'text/css', '.json': 'application/json' };
const server = http.createServer((req, res) => {
  let reqPath = req.url.split('?')[0];
  if (reqPath === '/') reqPath = '/index.html';
  if (BLOCKED_FILES.has(reqPath)) { res.writeHead(404); return res.end('Not found'); }
